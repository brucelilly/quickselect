...
* $Id: ~|^` @(#)    subtract.c copyright 2010 - 2016 Bruce Lilly.   \ subtract.c $
...
****************************** (end of license) ******************************/
/* $Id: ~|^` @(#)   This is subtract.c version 3.1 2016-05-31T16:37:39Z. \ $ */
/* You may send bug reports to bruce.lilly@gmail.com with subject "timespec" */
/*****************************************************************************/
/* maintenance note: master file  /src/radioclk/radioclk-1.0/lib/libtimespec/src/s.subtract.c */
...
#define SOURCE_MODULE "subtract.c"
...
/* subtract t2 from t1 placing normalized result in t3 */
void timespec_subtract(const struct timespec *t1, const struct timespec *t2, struct timespec *t3)
{
    ...
    t3->tv_sec = n1.tv_sec - n2.tv_sec;
    t3->tv_nsec = n1.tv_nsec - n2.tv_nsec;
    normalize_timespec_nonnegative(t3);
}
