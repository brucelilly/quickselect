...
* $Id: ~|^` @(#)    to_tmd.c copyright 2010 - 2016 Bruce Lilly.   \ to_tmd.c $
...
****************************** (end of license) ******************************/
/* $Id: ~|^` @(#)   This is to_tmd.c version 3.1 2016-05-31T16:37:39Z. \ $ */
/* You may send bug reports to bruce.lilly@gmail.com with subject "timespec" */
/*****************************************************************************/
/* maintenance note: master file  /src/radioclk/radioclk-1.0/lib/libtimespec/src/s.to_tmd.c */
...
void timespec_to_tmd(struct timespec *pts, struct tm *ptm, double *pd)
{
   if (NULL != pts) {
        time_t temp;
        double d;
        ...
        if (NULL != ptm)
            (void)gmtime_r(&(pts->tv_sec), ptm);
        temp = pts->tv_sec;
        pts->tv_sec = (time_t)0;
        d = timespec_to_double(pts);
        pts->tv_sec = temp;
        if (NULL != pd)
            *pd = d;
    }
