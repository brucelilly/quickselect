...
* $Id: ~|^` @(#)    normalize_nonneg.c copyright 2010 - 2016 Bruce Lilly.   \ normalize_nonneg.c $
...
****************************** (end of license) ******************************/
/* $Id: ~|^` @(#)   This is normalize_nonneg.c version 3.1 2016-05-31T16:37:39Z. \ $ */
/* You may send bug reports to bruce.lilly@gmail.com with subject "timespec" */
/*****************************************************************************/
/* maintenance note: master file  /src/radioclk/radioclk-1.0/lib/libtimespec/src/s.normalize_nonneg.c */
...
#define SOURCE_MODULE "normalize_nonneg.c"
...
/* normalize a timespec for non-negative nanoseconds */
void normalize_timespec_nonnegative(struct timespec *pts)
{
    if ((unsigned char)0U == timespec_initialized)
        initialize_timespec();
    while (0L > pts->tv_nsec) {
        pts->tv_nsec += 1000000000L;
        pts->tv_sec--;
    }
    while (1000000000L <= pts->tv_nsec) {
        pts->tv_nsec -= 1000000000L;
        pts->tv_sec++;
    }
}
