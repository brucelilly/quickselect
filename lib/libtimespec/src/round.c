...
* $Id: ~|^` @(#)    round.c copyright 2010 - 2016 Bruce Lilly.   \ round.c $
...
****************************** (end of license) ******************************/
/* $Id: ~|^` @(#)   This is round.c version 3.1 2016-05-31T16:37:39Z. \ $ */
/* You may send bug reports to bruce.lilly@gmail.com with subject "timespec" */
/*****************************************************************************/
/* maintenance note: master file  /src/radioclk/radioclk-1.0/lib/libtimespec/src/s.round.c */
...
/* round a timespec value to nearest interval nanoseconds */
void round_timespec(struct timespec *pts, long interval)
{
    if ((unsigned char)0U == timespec_initialized)
        initialize_timespec();
    normalize_timespec_nonnegative(pts);
    pts->tv_nsec += interval / 2L;
    pts->tv_nsec /= interval;
    pts->tv_nsec *= interval;
    normalize_timespec_nonnegative(pts);
}
