...
* $Id: ~|^` @(#)    normalize_near.c copyright 2010 - 2016 Bruce Lilly.   \ normalize_near.c $
...
****************************** (end of license) ******************************/
/* $Id: ~|^` @(#)   This is normalize_near.c version 3.1 2016-05-31T16:37:39Z. \ $ */
/* You may send bug reports to bruce.lilly@gmail.com with subject "timespec" */
/*****************************************************************************/
/* maintenance note: master file  /src/radioclk/radioclk-1.0/lib/libtimespec/src/s.normalize_near.c */
...
#define SOURCE_MODULE "normalize_near.c"
...
/* normalize a timespec to nearest second */
void normalize_timespec_nearest(struct timespec *pts)
{
    if ((unsigned char)0U == timespec_initialized)
        initialize_timespec();
    while (500000000L < pts->tv_nsec) {
