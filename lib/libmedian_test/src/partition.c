...
****************************** (end of license) ******************************/
/* $Id: ~|^` @(#)   This is partition.c version 1.4 dated 2017-11-03T21:27:21Z. \ $ */
...
#define MODULE_DATE "2017-11-03T21:27:21Z"
...
#if ASSERT_CODE + DEBUG_CODE
if ((DEBUGGING(PARTITION_DEBUG))&&(1000UL>beyond-first)) {
(V)fprintf(stderr, "/* %s: %s line %d: first=%lu, ipc=%lu, ipd=%lu, ipe=%lu, ipf=%lu, beyond=%lu */\n",
__func__,source_file,__LINE__,first,ipc,ipd,ipe,ipf,beyond);
}
#endif
...
#if ASSERT_CODE + DEBUG_CODE
if ((DEBUGGING(PARTITION_DEBUG))&&(1000UL>beyond-first)) {
(V)fprintf(stderr, "/* %s: %s line %d: first=%lu, mid=%lu, eq2=%lu, gt2=%lu, beyond=%lu */\n",
__func__,source_file,__LINE__,first,mid,eq2,gt2,beyond);
}
#endif
