...
(void)fprintf(stderr,"// %s: *px=%G, x=%d, *py=%G, y=%d\n",__func__,*((const double *)px),x,*((const double *)py),y);
    ...
    switch (antiqsort_type) {
        case DATA_TYPE_LONG :
            plong = (const int *)(antiqsort_base + x*antiqsort_typsz);
            a = *plong;
            plong = (const int *)(antiqsort_base + y*antiqsort_typsz);
            b = *plong;
        break;
        case DATA_TYPE_INT :
            plong = (const int *)(antiqsort_base + x*antiqsort_typsz);
...
static void duplicate_test_data(long *larray, int *array, double *darray, struct big_struct *big_array, size_t o, size_t n)
{
    long l;
    int i;
    size_t j;
...
static void restore_test_data(int type, size_t o, size_t n, long *larray, int *array, double *darray, struct big_struct *big_array)
{
    if (DATA_TYPE_LONG==type) {
        for (; o<n; o++)
            larray[o] = big_array[o].l;
    } else
        duplicate_test_data(larray, array, darray, big_array, o, n);
}

static unsigned int correctness_test(int type, size_t size, long *larray, int *array, double *darray, struct big_struct *big_array, const char *typename, int (*comparison_function)(const void *, const void *), const char *prog, unsigned int func, unsigned int testnum, size_t n, size_t count, unsigned int *psequences, unsigned int *ptests, int col, void (*f)(int, void *, const char *, ...), void *log_arg, unsigned char *flags)
{
    char buf[256];
        ...
        if (0U == errs) {
            ...
            switch (testnum) {
                case TEST_SEQUENCE_PERMUTATIONS :
                    ...
                    duplicate_test_data(larray,array,darray,big_array,0UL,n);
                break;
                case TEST_SEQUENCE_COMBINATIONS :
                    count = 0x01UL << n;
            ...
            for (m=o=0UL; m<count; m++) {
                switch (testnum) {
                    ...
                    default :
                        ...
                        /* copy test sequence to alternates */
                        duplicate_test_data(larray,array,darray,big_array,0UL,
                            n);
                    break;
                }
...
(void)fprintf(stderr, "%s line %d: quickselect returned\n", __func__, __LINE__);
            ...
            if (0U < errs) { /* error diagnostics */
                ...
                if (0U == rpt) {
                    restore_test_data(type,o,n,larray,array,darray,big_array);
                    if (MAX_ARRAY_PRINT > n) {
                        (void)fprintf(stderr, "initial %s array:\n", ptest);
                        switch (type) {
                            case DATA_TYPE_LONG :
...
static unsigned int timing_test(int type, size_t size, long *larray, int *array, double *darray, struct big_struct *big_array, const char *typename, int (*comparison_function)(const void *, const void *), const char *prog, unsigned int func, unsigned int testnum, size_t n, size_t count,  unsigned int *psequences, unsigned int *ptests, int col, double timeout, void (*f)(int, void *, const char *, ...), void *log_arg, unsigned char *flags)
{
...
static unsigned int correctness_tests(int type, size_t size, long *larray, int *array, double *darray, struct big_struct *big_array, const char *typename, const char *prog, unsigned int func, size_t n, size_t count, unsigned int *psequences, unsigned int *ptests, int col, double timeout, void (*f)(int, void *, const char *, ...), void *log_arg, unsigned char *flags)
{
    ...
    for (testnum=0U; TEST_SEQUENCE_COUNT>testnum; testnum++) {
        ...
        errs += correctness_test(type, size, larray, array, darray, big_array, typename, comparison_function, prog, func, testnum, n, count, psequences, ptests, col, f, log_arg, flags);
        if (0U != errs) break; /* abort on error */
    } /* loop through generated test sequences */
    return errs;
}

static unsigned int timing_tests(int type, size_t size, long *larray, int *array, double *darray, struct big_struct *big_array, const char *typename, const char *prog, unsigned int func, size_t n, size_t count, unsigned int *psequences, unsigned int *ptests, int col, double timeout, void (*f)(int, void *, const char *, ...), void *log_arg, unsigned char *flags)
{
    unsigned int errs=0U, testnum;
    int (*comparison_function)(const void *, const void *);
    const char *fname = function_name(func, n), *ftype = function_type(func, ptests);
    ...
    for (testnum=0U; TEST_SEQUENCE_COUNT>testnum; testnum++) {
        ...
        errs += timing_test(type, size, larray, array, darray, big_array, typename, comparison_function, prog, func, testnum, n, count, psequences, ptests, col, timeout, f, log_arg, flags);
        if (0U != errs) break; /* abort on error */
    } /* loop through generated test sequences */
    return errs;
}
...
static
unsigned int test_function(const char *prog, int *array, struct big_struct *big_array, double *darray, long *larray, unsigned int func, size_t n, size_t count, unsigned int *pcsequences, unsigned int *ptsequences, unsigned int *ptests, int col, double timeout, void (*f)(int, void *, const char *, ...), void *log_arg, unsigned char *flags)
{
    unsigned char c;
    ...
    for (c=0U; 1; c++) {
        ...
        if (0U == errs) {
            if (TEST_TYPE_CORRECTNESS == (TEST_TYPE_CORRECTNESS & *ptests)) {
                errs +=  correctness_tests(type, size, larray, array, darray, big_array, typename, prog, func, n, count, pcsequences, ptests, col, timeout, f, log_arg, flags);
            }
        }
        if (0U == errs) {
            if (TEST_TYPE_TIMING == (TEST_TYPE_TIMING & *ptests)) {
                errs +=  timing_tests(type, size, larray, array, darray, big_array, typename, prog, func, n, count, ptsequences, ptests, col, timeout, f, log_arg, flags);
            }
        }
    }
#if DEBUG_CODE
...
fprintf(stdout, "%s line %d: startn = %lu, incr = %lu, mult = %lu\n", __func__, __LINE__, startn, incr, mult);
    ...
    if ((0U==flags['A'])&&(0U==flags['F'])&&(0U==flags['I'])&&(0U==flags['L'])&&(0U==flags['S']))
        flags['A']=flags['F']=flags['I']=flags['L']=flags['S']=1U;
    ...
    for (n=startn; n<=ul; n=(n+incr)*mult) {
        ...
        if ((0U == errs) && (0U != flags['N'])) { /* test NetBSD qsort */
            ...
            if (0 != i) {
                ...
                tests = TEST_TYPE_MEDIAN | TEST_TYPE_PARTITION | TEST_TYPE_SORT ;
                errs += test_function(prog, array, big_array, darray, larray, FUNCTION_NBQSORT, n, count, &csequences, &tsequences, &tests, col, timeout, f, log_arg, flags);
            }
        }
        ...
        if ((0U == errs) && (0U != flags['g'])) { /* test glibc qsort */
            ...
            if (0 != i) {
                ...
                tests = TEST_TYPE_MEDIAN | TEST_TYPE_PARTITION | TEST_TYPE_SORT ;
                errs += test_function(prog, array, big_array, darray, larray, FUNCTION_GLQSORT, n, count, &csequences, &tsequences, &tests, col, timeout, f, log_arg, flags);
            }
        }
        ...
        if ((0U == errs) && (0U != flags['l'])) { /* test library qsort */
            ...
            if (0 != i) {
                ...
                tests = TEST_TYPE_MEDIAN | TEST_TYPE_PARTITION | TEST_TYPE_SORT ;
                errs += test_function(prog, array, big_array, darray, larray, FUNCTION_QSORT, n, count, &csequences, &tsequences, &tests, col, timeout, f, log_arg, flags);
            }
        }
