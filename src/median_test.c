...
#define OPTSTRING "aAbBc:C:dD:eFghHiIjk:KlLmM:nNoOPqQ:r:RsStT:wW:y:z"
#define USAGE_STRING     "[-a] [-A] [-b] [-B] [-c [c1[,c2[,c3[...]]]]] [-C sequences] [-d] [-D [n]] [-e] [-F] [-g] [-h] [-H] [-i] [-I] [-j] [-k col] [-l] [-L] [-m] [-M [m3,n]] [-n] [-N] [-o] [-O] [-P] [-q] [-Q timeout] [-r [n]] [-R] [-s] [-S] [-t] [-T sequences] [-w] [-W [f,c]] [-y [n]] [-z] -- [[start incr]] [size [count]]\n\
...
-r [n]\ttest introsort sort with optional insertion sort cutoff\n\
...
inline
#endif /* C99 */
void typeswap(char *pa, char *pb, size_t count, int swaptype)
{
/* Swap by chars uses supplied pointers */
# define CHAR_SWAP \
    while (1) {                   \
        t=*pa, *pa=*pb, *pb=t;    \
        if (0UL==--count) break;  \
...
/* Swap by other types uses typecast pointers */
# define GENERIC_SWAP \
    while (1) {                   \
        t=*px, *px=*py, *py=t;    \
        if (0UL==--count) break;  \
...
inline
#endif /* C99 */
void itypeswap(char *pa, char *pb, size_t count, int swaptype)
{
    nsw++;
    typeswap(pa,pb,count,swaptype);
}
...
static void print_long_array(long *target, size_t l, size_t u)
{
    ...
    for (i=l; i<=u; i++) {
        if (i != l)
            (void)fprintf(stderr, ", ");
        (void)fprintf(stderr, "%lu:%ld", i, target[i]);
    }
    (void)fprintf(stderr, "\n");
}
...
int cmp(const void *px, const void *py) /* per C standard */
{
    ...
    switch (qsort_type) {
        ...
        case DATA_TYPE_INT :
            ...
            y = *((const int *)py);
#if DEBUG_CODE
(void)fprintf(stderr,"// %s: x=%ld, y=%ld\n",__func__,x,y);
#endif
        break;
...
#if DEBUG_CODE
(void)fprintf(stderr,"// %s line %d: x=%ld, y=%ld\n",__func__,__LINE__,x,y);
#endif
    A(x<=antiqsort_gas);A(y<=antiqsort_gas);
    switch (antiqsort_type) {
        case DATA_TYPE_LONG :
...
#if DEBUG_CODE
(void)fprintf(stderr,"// %s line %d: a=%ld, b=%ld\n",__func__,__LINE__,a,b);
#endif
    /* brackets added BL */
...
void mbmqsort_internal(void *a, size_t n, size_t es, int (*compar)(const void *, const void *), int swaptype)
{
        char *pa, *pb, *pc, *pd, *pl, *pm, *pn;
        size_t d, r;
        int cmp_result;
...
loop:   if (n < (BM_INSERTION_CUTOFF)) {
                for (pm = (char *) a + es; pm < (char *) a + n * es; pm += es)
                        for (pl = pm; pl > (char *) a && compar(pl - es, pl) > 0;
                             pl -= es)
                                itypeswap(pl, pl - es,es,swaptype);
                return;
        }
        d = es * (n / 4UL);
        pm = (char *) a + d; /* 1/4 */
        ...
        /* pb-pa is the number of chars in the < block */
        r = bmmin(pa - (char *) a, pb - pa);
        if (0<r) itypeswap(a, pb-r, r, swaptype);
        /* after the swap (above), the start of the == section is at pb-r */
        /* compute the size of the block of chars that needs to be moved to put the > region after the upper == region */
        /* pn-pd-es (a.k.a. pu-pd) is the number of chars in the upper == block */
        /* pd-pc is the number of chars in the > block */
        r = bmmin((size_t)(pd - pc), pn - pd - es);
        if (0<r) itypeswap(pb, pn-r, r, swaptype);

        r = pb - pa; /* size (only) of < region */
        d = pd - pc; /* size (only) of > region */
        if (r<=d) {
                /* recurse on small pa..pb, iterate large */
                if (r>es) mbmqsort_internal(a, r/es, es, compar, swaptype);
                if (d>es) {
                    a = pn-d;
                    n = d/es;
                    goto loop;
                ...
                if (d>es) mbmqsort_internal(pn-d, d/es, es, compar, swaptype);
                if (r>es) {
                    n = r/es;
                    goto loop;
                }
...
void mbmqsort(void *a, size_t n, size_t es, int (*compar)(const void *, const void *))
{
    ...
    /* Determine size of data chunks to copy for element swapping.  Size is
       ...
       32-bit and 64-bit architectures.  Long and pointer type sizes vary
       by architecture.
    */
    s=typsz[swaptype=0]; /* double */
    if ((es<s)||(!(is_aligned(es,i=log2s[s])))||(!(is_aligned(a,i)))) {
        s=typsz[swaptype=3]; /* int */
        if ((es<s)||(!(is_aligned(es,i=log2s[s])))||(!(is_aligned(a,i)))) {
            s=typsz[swaptype=4]; /* short */
            if ((es<s)||(!(is_aligned(es,i=log2s[s])))
            ||(!(is_aligned(a,i))))
                swaptype=5; /* char */
        }
    }
    mbmqsort_internal(a, n, es, compar, swaptype);
}
    ...
    for (n=o=0UL; n<row_samples; n++,o+=sample_offset) {
        ...
        */
        ...
        if (0!=c) {
            ...
            if (0!=d) {
                if ((0<d)&&(0>c)) {
                    if (0>compar(pa,pc)) itypeswap(pb,pc,size,swaptype);
                    else itypeswap(pa,pb,size,swaptype);
                } else if ((0>d)&&(0<c)) {
                    if (0<compar(pa,pc)) itypeswap(pb,pc,size,swaptype);
                    else itypeswap(pa,pb,size,swaptype);
                }
...
void sqsort(void *base, size_t nmemb, size_t size, int (*compar)(const void *, const void *))
{
    ...
    int c, d, i;              /* general integer variables */
...
/* <- */return; /* Done; */
    ...
    while (1) { /* loop - partitioning */
        while ((pc<=pe)&&(0<=(c=compar(pivot,pc)))) { /* scan -> */
            if (c==0) { itypeswap(pivot=pb,pc,size,swaptype); pb+=size; }
            pc+=size;
        }
        while ((pc<=pe)&&(0>=(d=compar(pivot,pe)))) { /* scan <- */
            if (d==0) { pf-=size; itypeswap(pe,pivot=pf,size,swaptype); }
            pe-=size;
        }
        if (pc>pe) break;
        itypeswap(pc,pe,size,swaptype); /* <, > */
        pc+=size; pe-=size;
    }
...
inline
#endif /* C99 */
void sort5(char *pa, char *pb, char *pc, char *pd, char *pe, size_t size, int(*compar)(const void *, const void *), int swaptype)
{
    /* Batcher sorting network: 9 comparisons, 0-9 exchanges */
    if (0>compar(pa,pe)) itypeswap(pa,pe,size,swaptype);
    if (0>compar(pa,pc)) itypeswap(pa,pc,size,swaptype);
    if (0>compar(pb,pd)) itypeswap(pb,pd,size,swaptype);
    if (0>compar(pa,pb)) itypeswap(pa,pb,size,swaptype);
    if (0>compar(pc,pe)) itypeswap(pc,pe,size,swaptype);
    if (0>compar(pc,pd)) itypeswap(pc,pd,size,swaptype);
    if (0>compar(pb,pe)) itypeswap(pb,pe,size,swaptype);
    if (0>compar(pb,pc)) itypeswap(pb,pc,size,swaptype);
    if (0>compar(pd,pe)) itypeswap(pd,pe,size,swaptype);
}
...
inline
#endif /* C99 */
void dpqsort_internal(char *base, size_t nmemb, size_t size, int(*compar)(const void *,const void *), int swaptype)
{
    char *pa, *pb, *pivot1, *pivot2, *pm, *pn, *ps, *pt, *pu;
    size_t n, p, q, r, s, t, u, v, w, karray[2];
...
# define SAMPLE_SELECTION ADAPTIVE_SAMPLING
    ...
    while (1) {
        ...
        if (nmemb<=dp_cutoff) { /* insertion sort */
            for (pm=base; pm<pu; pm+=size)
                for (pa=pm; (pa>=base)&&(0<compar(pa,pb=pa+size)); pa-=size)
                    itypeswap(pa,pb,size,swaptype);
    /* <- */return; /* Done; */
        }
        ...
        for (r=((s+1UL)>>1),q=1UL,t=nmemb/s*size; q<r; q++) {
            u=q*t;
            v=q*size;
            if (pm-u>=base) itypeswap(pm-u,pm-v,size,swaptype);
            if (pm+u<pu) itypeswap(pm+u,pm+v,size,swaptype);
        }
        r=s>>1;
        q=(s+1UL)/6UL;
        karray[0]=r-q;
        ...
        quickselect(pn,n,size,compar,karray,2UL); /* bootstrap with quickselect */
#else
        ...
        pivot2=pm+t;
        ps=pivot2+t;
        sort5(pa,pivot1,pm,pivot2,ps,size,compar,swaptype);
#endif
#if DPDEBUG
...
print_int_array(base,0UL,nmemb-1UL);
        ...
        if (r>1UL) dpqsort_internal(pb,r,size,compar,swaptype);
        if (s>1UL) dpqsort_internal(pt,s,size,compar,swaptype);
        if (q<2UL) return;
        nmemb=q;
    }
}
...
void dpqsort(void *base, size_t nmemb, size_t size, int(*compar)(const void *, const void *))
{
    ...
    */
    ...
    dpqsort_internal(base, nmemb, size, compar, swaptype);
}
...
inline
#endif /* C99 */
void yqsort_internal(char *base, size_t nmemb, size_t size, int(*compar)(const void *, const void *), size_t y_div, int swaptype)
{
    int pcmp;
...
/* <- */return; /* Done; */
    ...
    if (0<pcmp) {
        itypeswap(base,pivot2,size,swaptype);
        itypeswap(pivot1,pr,size,swaptype);
    } else {
        itypeswap(base,pivot1,size,swaptype);
        itypeswap(pivot2,pr,size,swaptype);
    }
    ...
    for (pk=pl; pk<=pg; pk+=size) {
        if (0>compar(pk,pivot1)) itypeswap(pk,pl,size,swaptype), pl+=size;
        else if (0<compar(pk,pivot2)) {
            while ((pk<pg)&&(0<compar(pg,pivot2))) pg-=size;
            itypeswap(pk,pg,size,swaptype), pg-=size;
            if (0>compar(pk,pivot1)) itypeswap(pk,pl,size,swaptype), pl+=size;
        }
    }
    /* swaps */
    ...
    itypeswap(pivot1,base,size,swaptype);
    ...
    if (1UL<t) yqsort_internal(base,t,size,compar,y_div,swaptype);
    t=(pr-pivot2)/size; /* save size for protection comparison and recursion */
    A((2UL>t)||(pivot2+t*size<=pr));
    if (1UL<t) yqsort_internal(pivot2+size,t,size,compar,y_div,swaptype);
    /* equal elements */
    ...
    if ((0!=pcmp)&&(0UL<dist)) yqsort_internal(pl,dist+1UL,size,compar,y_div,swaptype);
}
...
void yqsort(void *base, size_t nmemb, size_t size, int(*compar)(const void *, const void *))
{
    ...
    */
    ...
    yqsort_internal(base, nmemb, size, compar, 3UL,swaptype);
}
/* ************************************************************** */
...
*/
static size_t introsort_insertion_cutoff = 16UL;
...
inline
#endif /* C99 */
void isort_internal(char *base, size_t nmemb, size_t size, int (*compar)(const void *, const void *), int swaptype)
{
    char *pa, *pb, *pc, *pu;

    for (pa=base,pu=base+(nmemb-1UL)*size; pa<pu; pa+=size)
        for (pb=pa; (pb>=base)&&(0<compar(pb,pc=pb+size)); pb-=size)
            itypeswap(pb,pc,size,swaptype);
}
...
void isort(char *base, size_t nmemb, size_t size, int (*compar)(const void *, const void *))
{
    ...
    isort_internal(base, nmemb, size, compar, swaptype);
}
...
{
#if 0
fprintf(stderr, "%s line %d: l=%lu, p=%lu, lc=%lu\n",__func__,__LINE__,l,p,l+((p-l)<<1)+1UL);
#endif
    return l + ((p-l)<<1) + 1UL;
}
...
{
#if 0
fprintf(stderr, "%s line %d: l=%lu, p=%lu, rc=%lu\n",__func__,__LINE__,l,p,l+(((p-l)+1UL)<<1));
#endif
    return l + (((p-l) + 1UL)<<1);
}
...
{
#if 0
fprintf(stderr, "%s line %d: l=%lu, c=%lu, p=%lu\n",__func__,__LINE__,l,c,l+((c-l-1UL)>>1));
#endif
    return l + ((c-l-1UL)>>1);
}
...
inline
#endif /* C99 */
size_t heap_siftdown(char *base, size_t l, size_t p, size_t u, size_t size, int(*compar)(const void *, const void *), int swaptype)
{
    size_t c, rc;

    /* pre-condition: heap(p+1..u) */
#if 0
fprintf(stderr, "%s line %d: l=%lu, p=%lu, u=%lu\n",__func__,__LINE__,l,p,u);
#endif
    for (; (c = left_child(l,p)) <= u; p = c) {
        rc=right_child(l,p);
#if 0
fprintf(stderr, "%s line %d: l=%lu, p=%lu, c=%lu, rc=%lu, u=%lu\n",__func__,__LINE__,l,p,c,rc,u);
#endif
        ...
        itypeswap(base+size*p, base+size*c, size, swaptype);
    }
    return p;
}
...
inline
#endif /* C99 */
void heapify_array(char *base, size_t r, size_t u, size_t size, int(*compar)(const void *, const void *), int swaptype)
{
    size_t p;

#if 0
fprintf(stderr, "%s line %d: base=%p,r=%lu,u=%lu)\n",__func__,__LINE__,base,r,u);
#endif
    for (p=parent(r,u); 1; p--) {
#if 0
fprintf(stderr, "%s line %d: calling heap_siftdown(base,r=%lu,p=%lu,u=%lu)\n",__func__,__LINE__,r,p,u);
#endif
        (void)heap_siftdown(base, r, p, u, size, compar, swaptype);
#if 0
print_int_heap(base,r,u);
#endif
        if (p == r)
            break;
    }
...
inline
#endif /* C99 */
size_t heap_delete_root(char *base, size_t r, size_t n, size_t size, int(*compar)(const void *, const void *), int swaptype)
{
    if (r != n) { itypeswap(base+size*r, base+size*n, size, swaptype); /* swap root and last */ }
    --n;
    (void)heap_siftdown(base, r, r, n, size, compar, swaptype);
    return n;
}
...
inline
#endif /* C99 */
void heap_sort_internal(char *base, size_t first, size_t beyond, size_t size, int(*compar)(const void *, const void *), int swaptype)
{
    size_t n;
...
#endif
    heapify_array(base, first, beyond-1UL, size, compar, swaptype);
#if 0
print_int_heap(base,first,beyond-1UL);
#endif
    for (n=beyond-1UL; first < n;)
        n = heap_delete_root(base, first, n, size, compar, swaptype);
}
...
inline
#endif /* C99 */
void heap_sort(char *base, size_t nmemb, size_t size, int(*compar)(const void *,const void *))
{
    int i, swaptype;
    size_t s;
    ...
    heap_sort_internal(base, 0UL, nmemb, size, compar, swaptype);
}
...
inline
#endif /* C99 */
void partition(char *base, size_t first, size_t beyond, char *pivot, size_t size, int(*compar)(const void *,const void*), int swaptype, size_t *peq, size_t *pgt)
{
...
inline
#endif /* C99 */
void introsort_loop(char *base, size_t first, size_t beyond, size_t size, int(*compar)(const void *, const void *), int swaptype, size_t depth_limit, size_t call_depth)
{
    size_t eq, gt;
    char *pivot;

#if 0
fprintf(stderr, "%s line %d: first=%lu,beyond=%lu,depth_limit=%lu, call_depth=%lu)\n",__func__,__LINE__,first,beyond,depth_limit,call_depth);
#endif
    while (beyond-first>introsort_insertion_cutoff) {
        if (0UL==depth_limit) {
#if 0
fprintf(stderr, "%s line %d: depth_limit=%lu)\n",__func__,__LINE__,depth_limit);
#endif
            heap_sort_internal(base,first,beyond,size,compar,swaptype);
            return;
        }
        depth_limit--;
        /* Introsort can probably be improved by improving the quality and
        ...
        */
        partition(base,first,beyond,pivot,size,compar,swaptype,&eq,&gt);
#if 0
fprintf(stderr, "%s line %d: pivot=%p[%lu], eq=%lu, gt=%lu)\n",__func__,__LINE__,pivot,(pivot-base)/size,eq,gt);
print_int_array(base,0UL,beyond-1UL);
#endif
        introsort_loop(base,gt,beyond,size,compar,swaptype,depth_limit,++call_depth);
        beyond=eq;
    }
}
...
void introsort(char *base, size_t nmemb, size_t size, int(*compar)(const void *,const void *))
{
    ...
    depth_limit=2UL*floor_lg(nmemb);
#if 0
fprintf(stderr, "%s line %d: depth_limit=%lu)\n",__func__,__LINE__,depth_limit);
#endif
    introsort_loop(base, 0UL, nmemb, size, compar, swaptype, depth_limit, 1UL);
    isort_internal(base,nmemb,size,compar,swaptype);
}
/* ************************************************************** */
...
static void random_fill(long *p, size_t count, int modulus)
{
    size_t j;
    ...
    if (NULL != p) {
        ...
        for (o=0UL,r=0UL; r<count; r++,o+=n) {
            switch (testnum) {
                ...
                case TEST_SEQUENCE_RANDOM_DISTINCT :
                    increment(&p[o],0UL,u,0);
                    (void)fisher_yates_shuffle(&(p[o]), n, sizeof(long), random64n, f, log_arg);
                break;
                case TEST_SEQUENCE_RANDOM_VALUES :
...
static void duplicate_test_data(long *larray, int *array, double *darray, struct big_struct *big_array, struct big_struct **parray, size_t o, size_t n)
{
    ...
    for (j=o; j<n; j++) {
        ...
        (void)snl(big_array[j].string, BIG_STRUCT_STRING_SIZE, NULL, NULL, l, 36,
...
static unsigned int correctness_test(int type, size_t size, long *larray, int *array, double *darray, struct big_struct *big_array, struct big_struct **parray, const char *typename, int (*comparison_function)(const void *, const void *), const char *prog, unsigned int func, unsigned int testnum, size_t n, size_t count, unsigned int *psequences, unsigned int *ptests, int col, void (*f)(int, void *, const char *, ...), void *log_arg, unsigned char *flags)
{
    ...
    if (0U == errs) {
        switch (type) {
            ...
            break;
            default:
                (void)fprintf(stderr, "%s: %s line %d: unrecognized data type %u\n", __func__, source_file, __LINE__, type);
            return ++errs;
        }
        ...
        for (n=0U,ul=sizeof(flags); n<ul; n++) {
            if (0U!=flags[n]) {
                switch (n) {
                    ...
                    break;
                    case 'r' :
                        (void)fprintf(stdout, "%sintrosort insertion sort cutoff = %lu\n", comment, introsort_insertion_cutoff);
                    break;
                    case 'W' :
                        (void)fprintf(stdout,
...
fprintf(stderr, "testing for %lu samples at nmemb >= %lu\n", sampling_table[x].samples, sampling_table[x].min_nmemb);
                    (void)test_function(prog, array, big_array, darray, larray, parray, FUNCTION_SQSORT, m, y, &cs, &ts, &tests, col, timeout, f, log_arg, flags);
